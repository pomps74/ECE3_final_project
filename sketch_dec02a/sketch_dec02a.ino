...
double var_weights[] = { -4, -3, -2, -1, 1, 2, 3, 4};
...
void loop()
{
  ...
  for (int i = 0; i < 8; i++) {
    ...
    var_error += var_weights[i] * ((double)sensorValues[i]);
  }
  var_error_avg = var_error * .125;
  for (int i = 0; i < 8; i++)
    std += ((double)sensorValue[i] * var_weights[i]) * ((double)sensorValue[i] * var_weights[i]) - (var_error * var_error);
  std *= .125;
  (prev_std + pow(prev_var_error, 2)) - pow(var_error_avg, 2) + (pow(var_error, 2) - pow(prev_std, 2) - pow(prev_var_error, 2))/9;
